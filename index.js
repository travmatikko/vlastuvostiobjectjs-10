...
function customObjectFromEntries(entries) {
  ...
  const CustomMap = entries.map(([key, value]) => {
    if (key === Number) {
      return key.toStrring();
    }
    return [key, value];
  });
...
function checkProperty(obj, prop) {
  ...
  const pop = checkProperty.hasOwnProperty("{ a: 1, b: 2, c: 3 }");
  return pop;
...
function filterObjectsByKey(arr, key) {
  ...
  // const result = key.filter((filterObjectsByKey) => a);
  // return result;
